...
REPO= OBJECTIVE= WIP=${FM_LIFECYCLE_WIP_LIMIT:-3}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --wip) WIP=${2:-}; shift 2 ;;
    -h|--help) echo "usage: fm-lifecycle-admit.sh --repo <repo> --objective <text> [--wip N]"; exit 0 ;;
...
for record in "$STATE"/*.lifecycle; do
  ...
  if [ "$objective" = "$OBJECTIVE" ]; then
    die "duplicate active objective for repo=$REPO (task=$(basename "$record" .lifecycle))"
  fi
  active=$((active + 1))
done
