...
There is no required third-party promotion proxy.
Use the repository's native lint, behavior tests, and CI checks, then open a normal pull request.
`no-mistakes` remains an optional local adapter for contributors who explicitly want it.

## Workflow

1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent (`git@github.com:kunchenguid/firstmate.git`).
2. Create a branch and make your changes.
3. Install and configure any optional local tools you prefer; no third-party gate initialization is required.
4. Commit your changes.
5. Push your branch to your fork or configured remote:

   ```sh
   git push origin HEAD
   ```

6. Open the pull request and wait for the repository-native CI checks.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - use the normal branch/PR flow and repository-native checks.
...
Crewmate validation follows the repository-native lint and behavior-test contract.
