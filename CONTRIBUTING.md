...
Changes to this repository use normal repository-native tests and CI.
Treehouse and no-mistakes are retired and are not required or supported.

## Workflow

1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent (`git@github.com:kunchenguid/firstmate.git`).
2. Create a branch and make your changes.
3. Run the repository-native checks described below.
4. Commit your changes.
5. Push to the configured remote when publication is requested:

   ```sh
   git push origin HEAD
   ```

6. Open a PR only when the repository workflow calls for one.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, and pinned shellcheck version), and CI runs it.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - use the repository-native checks and normal mainline workflow.
...
Firstmate is queue/context/status only in the active direct-mainline workspace mode.

Check and test the toolbelt before pushing:

```sh
for script in bin/*.sh bin/backends/*.sh; do bash -n "$script"; done   # syntax-check the toolbelt
bin/fm-lint.sh   # lint the toolbelt and behavior tests
for test_script in tests/*.test.sh; do bash "$test_script"; done   # behavior tests
